...
for (const marker of ["window.HHAccount.signOut()", 'setStack([{ view: "email" }])', "<AccountScreen"]) if (!app.includes(marker)) throw new Error(`account route missing: ${marker}`);
for (const marker of ["function openAccount()", "account && !account.guest", 'setStack([{ view: "email" }])']) if (!app.includes(marker)) throw new Error(`guest account routing missing: ${marker}`);
...
for (const marker of ["账号管理", "退出登录", "确认退出", "退出登录不会删除", 'role="dialog"']) if (!account.includes(marker)) throw new Error(`account screen missing: ${marker}`);
