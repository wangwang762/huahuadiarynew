...
function FlatGardenPlant({ plant, index, total, go }) {
  ...
  return (
    <button className="flat-garden-plant" onClick={() => go("plantDiary", plant)}
      aria-label={`打开${plant.name}的日记`} style={{ "--plant-delay": `${index * .045}s` }}>
  ...
  React.useEffect(() => {
    ...
    async function visitLoop() {
      ...
      while (alive) {
        ...
        const crowns = host && Array.from(host.querySelectorAll(".flat-garden-plant-art"));
...
function GardenRoomDecor({ weatherName }) {
  const wallSrc = weatherName === "晴"
  ...
  return <div className="garden-room-decor" aria-hidden="true">
    ...
    <img className="garden-generated-print" src="assets/garden-print-v1.webp" alt="" />
    <img className="garden-generated-chair" src="assets/garden-chair-v1.webp" alt="" />
  </div>;
}

function GardenScreen({ go }) {
  const plants = (window.PLANTS || []).slice(0, 20);
  ...
  const levelCount = Math.max(1, Math.ceil(plants.length / 5));
  const rows = Array.from({ length: levelCount }, (_, row) => plants.slice(row * 5, row * 5 + 5));

  return (
    <div ref={gardenRef} className={`flat-garden flat-weather-${weatherName}`}>
      <div className="flat-weather-zone" aria-hidden="true">
        {weatherName === "小雨" && Array.from({ length: 24 }).map((_, i) =>
          <i className="flat-raindrop" key={i} style={{ left: `${3 + (i * 23) % 96}%`, animationDelay: `${(i % 7) * -.18}s` }}></i>)}
        {weatherName === "晴" && <span className="flat-sun-glow"></span>}
        {weatherName === "多云" && <><span className="flat-cloud cloud-one"></span><span className="flat-cloud cloud-two"></span></>}
      </div>
      <GardenRoomDecor weatherName={weatherName} />
      <GardenVisitors hostRef={gardenRef} active={plants.length > 0} />

      <main className={`flat-garden-rack flat-rack-levels-${levelCount}`} aria-label="花园">
        {rows.map((rowPlants, rowIndex) => (
          <div className={`flat-garden-tier flat-tier-${5 - levelCount + rowIndex}`} key={rowIndex}>
            <div className={`flat-garden-residents flat-row-count-${rowPlants.length}`}>
              {rowPlants.map((plant, index) => <FlatGardenPlant key={plant.id} plant={plant}
                index={rowIndex * 5 + index} total={plants.length} go={go} />)}
              {!plants.length && rowIndex === 0 &&
                <button className="flat-first-plant" onClick={() => go("onboard")} aria-label="添加第一盆植物">
